...
from a2c_workspace.core.a2c_agent import A2CAgent
from src.core.llm_multiagent_reward_tuner import LLMRewardTuner

class SingleA2CRLController(BaseController):
    ...
    def step(self, traci_conn, metrics):
        ...
        if self.last_state is not None and self.training:
            # A2C calls for explicit reward calculation BEFORE storing,
            # because the buffer is cleared every episode anyway.
            local_metrics = rl_utils.get_local_metrics(traci_conn, self.ts_id, self.ts_info)
            q_val = local_metrics['avg_queue_length']
            w_val = local_metrics['incremental_waiting_time'] if local_metrics['incremental_waiting_time'] > 0 else local_metrics['total_accumulated_waiting_time']/100.0
            c_val = local_metrics['co2_emission_kg_per_s']
            
            alpha = self.weights.get('alpha', 0.6)
            beta  = self.weights.get('beta', 0.3)
            gamma = self.weights.get('gamma', 0.1)
            
            reward = -(alpha * q_val + beta * w_val + gamma * c_val)
...
def run_experiment(mode, out_file, episodes=10, duration=3600, use_gui=False):
    ...
    agent = A2CAgent(state_dim=5, action_dim=2) if mode != "baseline" else None
    tuner = LLMRewardTuner(timeout=60, max_delta=0.10) if mode == "a2c_llm" else None
    ...
    for ep in range(1, episodes + 1):
        if mode == "baseline":
            ...
            ctrl_class = SingleA2CRLController
            ctrl_kwargs = {
                "agent": agent,
                "training": True,
        ...
        # A2C: Learn at the end of the episode
        if agent and mode != "baseline":
            agent.learn_episode()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["baseline", "a2c", "a2c_llm"], required=True)
