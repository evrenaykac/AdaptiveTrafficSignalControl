...
from ppo_workspace.core.ppo_agent import PPOAgent
from src.core.llm_multiagent_reward_tuner import LLMRewardTuner

class SinglePPORLController(BaseController):
    ...
    def step(self, traci_conn, metrics):
        ...
        if self.last_state is not None and self.training:
            # PPO calls for explicit reward calculation BEFORE storing,
...
def run_experiment(mode, out_file, episodes=10, duration=3600, use_gui=False):
    ...
    agent = PPOAgent(state_dim=5, action_dim=2) if mode != "baseline" else None
    tuner = LLMRewardTuner(timeout=60, max_delta=0.10) if mode == "ppo_llm" else None
    ...
    for ep in range(1, episodes + 1):
        if mode == "baseline":
            ...
            ctrl_class = SinglePPORLController
            ctrl_kwargs = {
                "agent": agent,
                "training": True,
        ...
        # PPO: Learn at the end of the episode
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["baseline", "ppo", "ppo_llm"], required=True)
