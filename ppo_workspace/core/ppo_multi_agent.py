"""
PPO Multi-Agent Controller for 3x3 Traffic Grid.

Key difference from MultiAgentController (DQN):
- DQN: learns at every step (off-policy, replay buffer)
- PPO: collects experience during episode, learns ONCE at episode end (on-policy)
...
class PPOMultiAgentController:
    """Controls 9 PPO agents on a 3x3 intersection grid."""
    ...
    def __init__(
        ...
        self.agents              = agents_dict   # ts_id → PPOAgent
        ...
        for ts_id in self.ts_ids:
            ...
            if self.prev_step_info[ts_id] is not None:
                ...
                if self.training:
                    ...
                    # NOTE: agent.learn() is a no-op for PPO; update happens at episode end
    ...
    # ------------------------------------------------------------------
    # Call this at the END of every episode to trigger PPO updates
    # ------------------------------------------------------------------
    def finish_episode(self):
        """Triggers learn_episode() for every agent and returns avg loss."""
        losses = []
        for ts_id, agent in self.agents.items():
            loss = agent.learn_episode(last_value=0.0)
            if loss is not None:
                losses.append(loss)
