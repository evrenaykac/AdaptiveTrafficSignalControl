...
class DynamicSingleRLController(BaseController):
    ...
    def step(self, traci_conn, metrics):
        ...
        if self.last_state is not None and self.training:
            # Store tuple into Dynamic Replay Buffer (Stores uncalculated metrics)
            self.agent.store_transition(self.last_state, self.last_action, raw_m, current_state, False)
            # Pass current weights so buffer sample calculates reward correctly
...
def run_experiment(mode, out_file, episodes=10, duration=3600, use_gui=False):
    ...
    agent = DQNAgent(state_dim=5, action_dim=2, epsilon=1.0) if mode != "baseline" else None
    tuner = LLMRewardTuner(timeout=60, max_delta=0.10) if mode == "dqn_llm" else None
    ...
    for ep in range(1, episodes + 1):
        ...
        if tuner:
            ...
            # For multiagent tuner signature, we pass local_metrics. Single agent has 1 tls.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["baseline", "dqn", "dqn_llm"], required=True)
